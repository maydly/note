...
(function (root) {
  ...
  var OVERRIDES = [
    ...
    /* ── 명진 확정 260913 · OTT 쉐어 정산 ──
       명진이 OTT 요금을 대표로 내고 같이 보는 사람들이 몫을 보내주는 돈.
       버는 돈이 아니라 «구독료를 나눠 낸 것»이므로 수입 합계에서 뺀다(「이체」가 MOVE에 걸림). */
    { re: /서용채|허나영|김진영|전문기|황서윤|채하라|박성원/, cat: "이체·OTT쉐어", type: "in" },
    /* ── 명진 확정 260913 · 촬영과 무관한 입금 (캘린더 대조에서 빼기 위해 분류를 준다) ── */
    { re: /^감사수당$|^오입금$|^수수료$|^용돈$|^일금$/, cat: "수입·기타", type: "in" },
    { re: /^취소$|^환불$/,          cat: "환불·취소",      type: "in" },
    { re: /^식비$|청년냉면|하얀마늘|건곤일척|호산성/, cat: "식비·편의점·마트" },
    { re: /^쿠팡$/,                 cat: "쇼핑·생활" },
    { re: /^주연$|^태영$|^메이$|^촬영$/, cat: "수입·기타", type: "in" },
    /* ── 명진 확정 260912 · 미분류 정리 ── */
    // 고수빈 = 여자친구·동업자. 들어온 돈은 촬영 수입이 아니라 정산이므로 합계에서 뺀다.
    // (나가는 쪽은 명진이 앞서 「지인·가족송금=지출」로 확정해 둬서 건드리지 않는다)
    { re: /고수빈/, cat: "이체·지인정산", type: "in" },
    { re: /가불/, cat: "수입·급여/사업", type: "in" },
    { re: /9월김수연/, cat: "수입·촬영/사업", type: "in" },
    { re: /캐[쉬시]백/, cat: "수입·이자/환급", type: "in" },   // 「캐시백」 표기도 함께
    { re: /님환불/, cat: "환불·취소", type: "out" },
    { re: /코지동물의료센터/, cat: "의료·약국" },
    { re: /산부인과/, cat: "의료·약국" },
    { re: /잇츠아이티/, cat: "쇼핑·생활" },
    { re: /현대토이즈/, cat: "쇼핑·생활" },
    { re: /교보문고/, cat: "쇼핑·생활" },
    { re: /\(주\)보배/, cat: "쇼핑·생활" },   // 그냥 /보배/면 «이보배» 같은 사람 이름을 가로챔
    { re: /타미/, cat: "쇼핑·생활" },
    { re: /크린토피아/, cat: "쇼핑·생활" },
    { re: /막걸리하우스/, cat: "식비·편의점·마트" },
    { re: /담솥/, cat: "식비·편의점·마트" },
    { re: /미니마트/, cat: "식비·편의점·마트" },
    { re: /하남시청/, cat: "식비·편의점·마트" },
    { re: /투썸/, cat: "식비·편의점·마트" },
    { re: /더벤티/, cat: "식비·편의점·마트" },
    { re: /비지에프/, cat: "식비·편의점·마트" },
    { re: /워커스하이/, cat: "식비·편의점·마트" },
    { re: /현장발권-S/, cat: "교통" },
    { re: /코레일유통/, cat: "교통" },
    { re: /아마노/, cat: "교통" },
    { re: /강남파크선학/, cat: "교통" },
    { re: /에스엠하이플러스/, cat: "차량·주유" },
    { re: /하나머니/, cat: "간편결제·페이" },
    { re: /피씨/, cat: "오락·게임" },
    { re: /인터내셔널브이/, cat: "생활지출" },
    { re: /원넷ARS/, cat: "생활지출" },
    { re: /보루 부평역/, cat: "생활지출" },
    { re: /하나카드/, cat: "생활지출", type: "out" },
    { re: /남촌도림동행정복지/, cat: "생활지출" },
    { re: /우정사업본부/, cat: "생활지출" },
    { re: /서울판지/, cat: "생활지출" },
  ];
  var INTERNAL = [
